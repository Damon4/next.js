...
describe.each([
...
])('SCSS Module Sourcemaps ($dependencies)', ({ dependencies, nextConfig }) => {
  ...
  ;(isNextDev ? describe.skip : describe)('Production only', () => {
    it('maps CSS module declarations to original SCSS lines', async () => {
      ...
      try {
        ...
        expect(selectorOriginal.source).toContain('index.module.scss')
        expect(selectorOriginal.line).toBe(3)

        const colorOriginal = findOriginalLine(consumer, cssContent, /color:/)

        expect(colorOriginal.source).toContain('index.module.scss')
        ...
        expect(paddingOriginal.source).toContain('index.module.scss')
        ...
        expect(backgroundOriginal.source).toContain('index.module.scss')
