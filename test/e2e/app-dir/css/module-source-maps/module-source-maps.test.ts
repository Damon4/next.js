...
describe('CSS Module Sourcemaps', () => {
  ...
  ;(isNextDev ? describe.skip : describe)('Production only', () => {
    it('maps CSS module declarations to original CSS lines', async () => {
      ...
      try {
        ...
        expect(selectorOriginal.source).toContain('index.module.css')
        expect(selectorOriginal.line).toBe(1)

        const colorOriginal = findOriginalLine(consumer, cssContent, /color:/)

        expect(colorOriginal.source).toContain('index.module.css')
        expect(colorOriginal.line).toBe(2)
        ...
        expect(paddingOriginal.source).toContain('index.module.css')
        expect(paddingOriginal.line).toBe(3)
        ...
        expect(backgroundOriginal.source).toContain('index.module.css')
        expect(backgroundOriginal.line).toBe(4)
      } finally {
        consumer.destroy?.()
      }
    })
